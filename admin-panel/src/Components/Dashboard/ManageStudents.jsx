...
import { fetchStudents, addStudent, deleteStudent } from "../../store/slices/studentSlice";
...
export default function ManageStudents() {
  ...
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", rollNumber: "" });
  ...
  const handleSubmit = () => {
    ...
    setModalOpen(false);
    setForm({ firstName: "", lastName: "", email: "", rollNumber: "", classId: "" });
  };

  const columns = ["First Name", "Last Name", "Email", "Roll Number", "Class", "Actions"];
  ...
  return (
    <div onDragEnter = {handleDrag} onDragOver = {handleDrag} onDragLeave = {handleDragLeave} onDrop = {handleDrop}>
      <div className="mb-4">
        ...
        {file && (
          ...
          onClick={() => {
            if (!id) {
              ...
              dispatch(uploadExcel({file, id})).unwrap(); // dispatch thunk
              setFile(null); // clear after upload if needed
            }
          }}
      ...
      <ModalForm title="Add Student" isOpen={isModalOpen} onClose={() => setModalOpen(false)} onSubmit={handleSubmit}>
        <input
          type="text"
