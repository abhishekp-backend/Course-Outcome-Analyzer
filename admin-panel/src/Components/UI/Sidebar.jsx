...
export default function Sidebar({ active }) {
  ...
  const { years, loading } = useSelector((state) => state.academicYear)
  const menuItems = [
  ...
  return (
    <aside className="w-64 bg-white text-gray-800 flex flex-col min-h-screen shadow-md">
      <h1 className="text-2xl font-bold p-6 border-b border-gray-200 m-auto">Admin Panel</h1>
      <select className="w-[80%] border rounded-sm m-auto p-1 ">
        {
          Array.from(years).map(e=>{
            return (
              <option key={e.year} value={e.year}>{e.label}</option>
            )
          })
        }
      </select>
        ...
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full text-left px-4 py-2 rounded transition ${
              active === item.path || active === item.path+"/" ? "bg-red-600 text-white" : "hover:bg-red-100 text-gray-800"
            }`}
