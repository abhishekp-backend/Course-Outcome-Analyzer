...
// Fetch faculty
export const fetchFaculty = createAsyncThunk("faculty/fetchFaculty", async (_, { rejectWithValue }) => {
  try {
...
// Update faculty
export const updateFaculty = createAsyncThunk("faculty/updateFaculty", async ({ id, data }, { rejectWithValue }) => {
  try {
...
// Delete faculty
export const deleteFaculty = createAsyncThunk("faculty/deleteFaculty", async (id, { rejectWithValue }) => {
  try {
...
const facultySlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateFaculty.rejected, (state, action) => { state.error = action.payload; })

      .addCase(deleteFaculty.fulfilled, (state, action) => {
        state.list = state.list.filter((f) => f.id !== action.payload);
      })
      .addCase(deleteFaculty.rejected, (state, action) => { state.error = action.payload; });
  },
});
